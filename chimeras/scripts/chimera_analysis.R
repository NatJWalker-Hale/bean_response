...
setwd("D:/Dropbox/cary_projects/DODA/cary/bean_rebuttal/")
df <- read.csv("chimera_data.csv", header=T)
str(df)
df$sequence <- as.factor(df$sequence)

ggplot(aes(x = sequence, y=log(fluorescence)), data=df) +
...
fit1.log <- lm(log(fluorescence) ~ sequence, data=df) 
plot(fit1.log) # assumptions look okay
fit1.aov.log <- aov(log(fluorescence) ~ sequence, data=df)
summary(fit1.log)
summary(fit1.aov.log)
anova(fit1.log)
...
set.seed(12345)
summary(glht(fit1.aov.log, linfct=mcp(sequence="Dunnett")))
...
df$sequence <- relevel(df$sequence, "yRG004")
set.seed(12345)
fit1.aov.log.bvda2.ref <- aov(log(fluorescence) ~ sequence, data=df)
summary(glht(fit1.aov.log.bvda2.ref, linfct=mcp(sequence="Dunnett")))
df$sequence <- relevel(df$sequence, "yRG003") # reverse change
...
set.seed(12345)
summary(glht(fit1.aov.log, linfct=mcp(sequence="Dunnett"), vcov=vcovHC))
...
set.seed(12345)
fit1.aov.log.bvda2.ref <- aov(log(fluorescence) ~ sequence, data=df)
summary(glht(fit1.aov.log.bvda2.ref, linfct=mcp(sequence="Dunnett"), vcov=vcovHC))
df$sequence <- relevel(df$sequence, "yRG003") # reverse change
